...
DEFAULT_DOWNLOAD_ROOT = Path(r"C:\Users\SSAFY\Downloads\finals")
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Download all final images for a session from S3."
    )
    parser.add_argument("session_id", help="Session ID used in the final S3 key.")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=DEFAULT_DOWNLOAD_ROOT,
        help="Directory where the session final files will be downloaded.",
    )
    return parser
...
def _list_final_keys(s3_client, bucket_name: str, session_id: str) -> list[str]:
    prefix = f"ai-finals/{session_id}/"
...
def main() -> None:
    ...
    final_keys = _list_final_keys(s3_client, bucket_name, args.session_id)
    if not final_keys:
        raise FileNotFoundError(
            f"No finals found for session_id={args.session_id} in bucket {bucket_name}."
        )
